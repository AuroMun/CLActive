...
FASTTEXT_QUANTIZE = """
{command} quantize -input "{train_file}" -output "{model_file}" -qnorm -retrain -epoch 1 -cutoff 100000
"""
FASTTEXT_PREDICT_QUANTIZE = """
{command} predict "{model_quant}" "{test_file}" > "{test_predict}"
"""
FASTTEXT_PROBS_QUANTIZE = """
{command} predict-prob "{model_quant}" "{full_train_file}" {num_classes} \
> "{train_probs}"
"""
...
def fasttext(executable: str,
    ...
    for next_size in sizes[1:]:
        ...
        _ = shuffle_and_index(train_dataset, current_path, kept=new_indices)
        utils.save_index(new_indices, round_dir,
                         'new_indices_{}.index'.format(len(new_indices)))
...
def train_fastText(command: str, train_file:  str, full_train_file: str,
                   ...
                   bucket: int = 10000000, quantize: bool = False, verbose=True
    ...
    if quantize == False:
        ...
        command = FASTTEXT_QUANTIZE.format(**config)
        run_command(command, verbose=verbose)

        command = FASTTEXT_PROBS_QUANTIZE.format(**config)
        run_command(command, verbose=verbose)
    ...
    if test_file is not None:
        ...
        if quantize == False:
            ...
            command = FASTTEXT_PREDICT_QUANTIZE.format(**config)
            run_command(command, verbose=verbose)
