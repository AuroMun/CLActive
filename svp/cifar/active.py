...
def active(run_dir: str = './run',
    ...
    memory_ratio : int, default 1
        The ratio of prior samples taken to sample size
    samples_ratio: int, default 1
        The ratio of samples used to sample size
    """
    ...
    if precomputed_selection is not None:
        ...
        for selection_size in rounds:
            old_labels = labeled
            old_labeled_subset = np.random.choice(labeled, size=min(int(selection_size*memory_ratio), labeled.size))
            ...
            # model, stats = proxy_generator.send(labeled)
            labels_to_send = np.concatenate((old_labeled_subset, np.random.choice(np.setdiff1d(labeled, old_labels), int(selection_size*samples_ratio) )))
            model, stats = proxy_generator.send(labels_to_send)
            utils.save_result(stats, os.path.join(run_dir, 'proxy.csv'))
