...
def run_training(model: nn.Module, optimizer: Optimizer, criterion: nn.Module,
    ...
    #Use OneCycleLR only if it's not an eval model! Check that the number of epochs isn't 15 (fastText) or 181 (everything else)
    print("Sum of epochs:", str(sum(epochs)))
    if sum(epochs) != 181 and sum(epochs) != 15:
        print("Using OneCycleLR!")
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.2, steps_per_epoch=len(train_loader), epochs=sum(epochs))
        for group in optimizer.param_groups:
            group['lr'] = learning_rates[0]
    else:
        print("Using normal LR")
    ...
    for nepochs, learning_rate in zip(epochs, learning_rates):
        ...
        if sum(epochs) == 181 or sum(epochs)==15:
            for group in optimizer.param_groups:
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if sum(epochs) != 181 and sum(epochs)!=15:
                global_step, train_accs, epoch_train_time = run_epoch(
                    ...
                    batch_callback=batch_callback, fp16=fp16, scheduler=scheduler)
            else:
                global_step, train_accs, epoch_train_time = run_epoch(
                    epoch, global_step, model, train_loader, device,
                    criterion=criterion, optimizer=optimizer,
