...
def generate_models(create_graph: CreateGraph,
    ...
    while True:
        model_old, optimizer_old = model, optimizer
        ...
        if model_old is not None: model, optimizer = model_old, optimizer_old
        ...
        if use_cuda and not isinstance(model, torch.nn.DataParallel):
